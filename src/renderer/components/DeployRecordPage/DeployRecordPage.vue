  ...
  <div>
    <el-collapse accordion>
      ...
      >
        <template slot="title">
          <label
            style="max-width: 40%; display: inline-block; margin-right: 10px"
            v-ellipsis.middle
            >{{ deployRecord.sourceFilepath }}</label
          >
          <i class="el-icon-right" style="color: green; font-size: 20px"></i>
          <label
            style="max-width: 40%; display: inline-block; margin-left: 10px"
            v-ellipsis.middle
            >{{ deployRecord.destFilepath }}</label
          >
        </template>
        <div
          v-for="file in deployRecord.filename"
